...
app.config["SQLALCHEMY_DATABASE_URI"] = 'postgresql://postgres:12345@localhost:5432/MezzproFinalDemo'
...
class UserRole(Enum):
    ...
    BUYERS_ADMIN = "Buyers Admin"
    SELLER_ADMIN = "Seller Admin"
    SUPPLIER_TO_SELLER_ADMIN = "Suppliers to Seller Admin"
    ESCROW_BANK_ADMIN = "Escrow Bank Admin"
    LENDER_ADMIN = "Lender Admin"
    SELLER = "Seller"
    SUPPLIER = "Supplier"
    ESCROW_BANK = "Escrow Bank"
    LENDER = "Lender"
    BUYER = "Buyer"
...
def register_user():
    ...
    gstin = data.get('gstin')  # Renamed from 'tin' to 'gstin'
    ...
    if not (username and first_name and last_name and email and password and address and mobile_no and confirmPassword and bank_name and branch and ifsc_code and account_number and company_name and gstin and pan_number and metamask_address):
...
def parse_gpt_response(gpt_response):
    ...
    try:
        due_date = dateutil.parser.parse(due_date_str).strftime('%d-%m-%Y') if due_date_str else None
    except:
...
def upload_invoice():
    ...
    # Send extracted text to OpenAI using the correct chat completions endpoint
    response = openai.ChatCompletion.create(
        model="gpt-4-1106-preview",  # Update the model name if needed
        messages=[
            {"role": "system", "content": "Extract the invoice ID, due date, and total amount from the following text.Convert any dates into the format 'dd-mm-yyyy'. If the due date is written in text form, such as '30th June 2023', it should be converted to '30-06-2023' ."},
            {"role": "user", "content": extracted_text}
        ]
    )

    # Correctly accessing the response for chat models
    gpt_response = response['choices'][0]['message']['content'].strip()

    # Parsing the GPT response
    invoice_id, due_date, total_amount = parse_gpt_response(gpt_response)
...
def approve_invoice(invoice_id):
    ...
    # Update the approval status and the invoice status
    invoice.approval_status = True
    invoice.status = InvoiceStatus.APPROVED  # Assuming you have a status column and an InvoiceStatus Enum
